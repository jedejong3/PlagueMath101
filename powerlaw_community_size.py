...
def build_graph(nws_n, nws_k, nws_p, lnfa_c, lnfa_sigma, lnfa_num_steps):#, a):
    ...
    for n in range(lnfa_num_steps):
        ...
        county_size_list.append(nws_k + int(fitness_list[-1]*nws_n)+1)
        print(county_size_list[n])
        H = nx.newman_watts_strogatz_graph(county_size_list[n], nws_k, nws_p)
        H = nx.relabel_nodes(H, lambda x: x + G.order())
        G = nx.union(G, H)
        H_summary.add_node(n)
        if n > 0:
            to_add_index = random.choices(range(0,n), weights=fitness_list[:-1], k=lnfa_c)
    ...
    return G,H_summary, county_size_list
...
avg_safety = 10
G, H_summary, county_size_list = build_graph(smallworld_size, smallworld_k_nearest_neighbors, smallworld_odds_of_rewire, powerlaw_num_connections,
...
nx.draw(H_summary, node_color=colors, node_size=county_size_list)
...
for s in range(len(safety_generator_list)):
    ...
    for h in range(powerlaw_num_nodes):
        summs.append(sim.summary(nodelist=range(visited, visited+county_size_list[h])))
        individual_summary = []
